...
void Player::SetDims( Vector3d& _halfwidths ) {
	phys->halfwidths.Set( _halfwidths );
}

void Player::SetPos( Vector3d& _center ) {
...
void Player::TurnOnMoveFlag( int flag ) { m_nMoveFlags |= (1<<flag); }
void Player::TurnOffMoveFlag( int flag ) { m_nMoveFlags &= ~(1<<flag); }

void Player::RotateOnYaw( float fDeltaYaw ) {
	m_Yaw += fDeltaYaw;
}

void Player::RotateOnPitch( float fDeltaPitch ) {
	m_Pitch += fDeltaPitch;
}
