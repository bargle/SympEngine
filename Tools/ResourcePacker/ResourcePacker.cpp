...
int main(int argc, char** argv)
{
	...
	// Save the file header position for later.
	int nFileTablePosition = nFilePointerCur;

	int nFileTableSize = (sizeof(ResourceFileEntry) *fileHeader.nNumFiles);
	nFilePointerCur += nFileTablePosition;
	...
	// Write the file entries. //first pass... these are updated and written again later
	pFile->Write(&fileTable[0], (sizeof(ResourceFileEntry) *fileHeader.nNumFiles));

	//int nFileDataPointer = nFilePointerCur;
	...
	while(iter != fileTable.end())
	{
		...
		if(NULL != pFileEntry)
		{
			...
			int nZSize = (*iter).nFileSize + ((*iter).nFileSize / 8);
			uint8* pBytesZ = new uint8[nZSize];

			// Read in the file contents into our buffer.
			pFileEntry->Read(pBytes, (*iter).nFileSize);

			CCompressor compressor;
			if(compressor.CompressBuffer((char*)pBytesZ, nZSize, (char*)pBytes, (*iter).nFileSize))
			{
				//compress...
				(*iter).nFlags |= RESOURCE_ENTRY_FLAG_COMPRESSION;
				(*iter).nCompressedFileSize = nZSize;
				(*iter).nFilepointer = nFilePointerCur;
				nFilePointerCur += nZSize;

				g_nFilesizeZ += nZSize;
							
				// Write it out to the resource file.
				pFile->Write(pBytes, (*iter).nCompressedFileSize);
			}
			else
			{
				(*iter).nFileSize = 0;
				printf("Failed to compress...\n");
				//FIXME: do more interesting error handling here...
			}
	...
	pFile->SeekTo(nFileTablePosition);
	pFile->Write(&fileTable[0], (sizeof(ResourceFileEntry) *fileHeader.nNumFiles));
	...
	printf("  Total uncompressed file size: %d (%dK)\n", g_nFilesize, g_nFilesize/1024);
	printf("  Total file size: %d (%dK)\n", g_nFilesizeZ, g_nFilesizeZ/1024);
