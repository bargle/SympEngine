...
*/
	...
	public:
		...
		float TimeInSeconds()
		{
			return ((float)(m_nStopTime.QuadPart - m_nStartTime.QuadPart) / (float) m_nFrequency.QuadPart * 1000.0f) / 1000.0f;
		}
	...
	public:
		...
		float TimeInMilliseconds()
		{
			return (float)(((double)_tend.tv_sec + (double)_tend.tv_usec/(1000*1000)) - ((double)_tstart.tv_sec + (double)_tstart.tv_usec/(1000*1000)));
		}

		float TimeInSeconds()
		{
			return ((float)(((double)_tend.tv_sec + (double)_tend.tv_usec/(1000*1000)) - ((double)_tstart.tv_sec + (double)_tstart.tv_usec/(1000*1000)))) / 1000.0f;
		}
