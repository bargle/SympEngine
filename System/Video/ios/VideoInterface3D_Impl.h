...
public:
	...
	virtual HTEXTURE	CreateTexture( const char* name );
	virtual HTEXTURE	CreateTextureFromMemory(const char* name, uint8* buffer, int width, int height, TextureFormat_t format);
	virtual GEN_RESULT	DeleteTexture(HTEXTURE hTexture);
	virtual HSHADER		CreateShaderProgram( const char* name, const char* vs, const char* fs ){ return 0; }
	virtual GEN_RESULT	DeleteShaderProgram( HSHADER hShader ){ return RESULT_ERROR; }
	virtual GEN_RESULT	InstallShaderProgram( HSHADER hShader ){ return RESULT_ERROR; }
