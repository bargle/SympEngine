...
int ImageLoadPNG_3(const char *filename, ImageData *image, bool& hasAlpha) {
	...
	image->sizeX = info_ptr->width;
	
	image->sizeY = info_ptr->height;
...
HTEXTURE IVideoInterface3D::CreateTexture(const char* name, TextureFormat_t format){
	...
	//else load it...
	bool hasAlpha = false;
	if (ImageLoadPNG_3(name, &(tex.image), hasAlpha)) { //FIXME: load via PNGs and DON'T force 128x128 RAW RGB textures...
		// Create Texture
...
HFONT2D IVideoInterface3D::CreateFont(const char* name, int width, int height, float char_width, float char_height, int rows, int columns) {
...
GEN_RESULT IVideoInterface3D::DrawText(HFONT2D hFont, const char* text, float x, float y) {
